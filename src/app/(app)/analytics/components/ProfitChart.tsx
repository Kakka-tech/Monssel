...
export default function ProfitChart() {
  ...
  return (
    <div className="bg-white dark:bg-[#1E1F20] border border-[#ECEDEE] dark:border-[#2E2E2E] rounded-xl p-5 space-y-1">
      <h3 className="text-sm font-semibold text-[#1E1F20] dark:text-white">Profit Analysis</h3>
      <p className="text-xs text-[#707375] dark:text-[#A0A0A0]">Net profit breakdown by period</p>
      <div className="h-52 mt-4">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={DATA} margin={{ top: 4, right: 4, left: -16, bottom: 0 }} barSize={28}>
            ...
            <YAxis tickFormatter={formatY} tick={{ fontSize: 11, fill: tickColor }} axisLine={false} tickLine={false} />
            <Tooltip
              formatter={(value: number | undefined) => [`${symbol}${(value ?? 0).toLocaleString()}`, "Profit"]}
